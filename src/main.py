...
from helpers import print_task_header, ReviewDataset, printProgressBar, make_predictions, get_cm_as_dict, \
    print_subtask_header, read_base_data, get_class_report_as_dict
...
title = f"Fine-tuning and classification for {settings.model_name} with {review_lengths} data points"
...
for length in review_lengths:
    ...
    test_texts, val_texts, test_labels, val_labels = train_test_split(test_data, test_labels, test_size=.5)
    t_end = time.time()
    ...
    helpers.write_dict_to_json(name="testing", path=final_model_path, results=test_dict)
